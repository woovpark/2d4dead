...
// 9 맵 기본 메카닉 제작
// 9.1 입/출구는 바리케이드에 막혀있음 OK
// 9.2 맵에 들어 왔을때 타이머가 작동하여 5분후에 바리케이드가 폭발하여 출구가 노출된다. OK
// 9.2.1 폭발 이펙트 필요
// 9.3 휴게소 전용 맵 (탐사 전용 - 넓고 숨어있는 아이템들이 분포되어 있음) / 전투 전용 맵 (좁고 장애물이 거의 없는 상태에서 좀비들이 마구 쏟아짐)
// 두가지 맵 묶음을 갖고 장소에 맞게 랜덤하게 배치 OK
// 9.4 휴 - 전 - 휴 - 전 의 패턴을 구현 OK / 마지막 스테이지를 결정(전10에서 탈출? 휴11에서 탈출?)
// 9.6 로그라이크 맵 구성 (새 맵은 랜덤 생성, 지나간 맵은 정보를 저장하고 있어야 한다) 
// // 휴1(정해진 튜토리얼 맵) - 전1 - 휴2 - 전2 - 휴3 - 전3 -... - 휴10 - 전10(헬기장, 정해진 맵)
// // 20 개의 맵 기억 공간을 확보, 타입: 아직 안 간 맵, 이미 방문한 맵(루팅 아이템 정보도 기억해 놓는다)

// 10 아이템 루팅 시스템 OK
// 10.0.1 루팅 아이템 프리팹 만들기 - 경험치 아이템, 소총 총알 아이템(총기), 체력회복 아이템 OK
// 10.1 맵에 아이템 뿌릴 때 위치 중복 체크
