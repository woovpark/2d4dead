...
// 7.4 리로드를 구현한다
// 7.4.1 총알의 총 소지 수 지정, 및 탄창 시스템 구현 OK
// 7.4.2 리로드 키를 누르면 바로 리로딩 (수동 리로딩)
// 7.4.3 리로딩 중에 무기를 바꾸면 리로딩 취소(탄창은 0)
// 7.4.4 무기를 바꾼 직후 탄창이 0인데 가방에 총알이 있으면 즉시 자동 리로딩 (리로딩중 발사 안됨)
// 7.4.6 리로딩 비주얼 실제로 구현
// 7.5 초기 총알 지급방식 결정 - 탄창과 가방에 총알을 채워줄것인가 아닌가
...
// 10 아이템 루팅 시스템 OK
// 10.0.1 루팅 아이템 프리팹 만들기 - 경험치 아이템, 소총 총알 아이템(총기), 체력회복 아이템
